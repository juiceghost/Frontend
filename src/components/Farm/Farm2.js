...
const Farm2 = ({ farm, prices, userInfo, forceUpdate, active, stakeOnly }) => {
    ...
    if ((stakeOnly && userInfo && isZero(userInfo.stakedBalance)) || (active && !isZero(multiplier)) || (!active && isZero(multiplier))) {
        return (<></>)
    }
    ...
    return (<>
        ...
        <div className="farm">
            ...
            {account && <div className="earn-wrap">
                <div className="harvest">
                    ...
                    <div className="h-button" onClick={handleHarvest}>Harvest</div>

                </div>
            </div>}

            {account && <div className="action-farm">
                {!requestedApproval || allowance.gt(0) ? <div style={{ display: "flex", justifyContent: "space-between" }}>
                    {farm.isDisable ?
                        <div className="lq-button  blue-button" style={{ width: "45%" }}>
                            Stake
                        </div> :
            ...
            <div className="apr-wrap">
                <p className="apr">
                    ...
                    <span> {prices && priceQuoteToken !== 0 && !isZero(lpTotalInQuoteToken) && !isNaN(poolWeight) ?
                ...
                {account && <p className="apr">
                    <span className="a-title"> Your Stake</span>
                    <span>{isZero(stakedBalance) ? 0 :
                        new BigNumber(stakedBalance).isLessThan(0.00001) ? "<0.00001" :
                            new BigNumber(stakedBalance).toFormat(5)} {farm.lpSymbol}</span>
                </p>}


            </div>
            <div className="details-btn" onClick={() => setShowDetails(!showDetails)}>
                <div className="d-title" >
                    <p>Details</p>
                    <img src={`/img/svg/${showDetails ? "mines" : "plus"}.svg`} style={{ marginLeft: "9px" }} alt="plus" />
                </div>

                <div className="d-content" style={{ display: showDetails ? "block" : "none" }}>
                    <div className="item">
                        <p>Total Staked</p>
                    ...
                    <div className="item">
                        ...
                        <p>${(priceQuoteToken && !isZero(totalStaked)) ? lpTotalInQuoteToken.times(priceQuoteToken).div(totalStaked).toFormat(1) : 0} </p>
                    </div>
                    <div className="item">
                        <p>TVL</p>
                        <p>${!priceQuoteToken ? 0 : lpTotalInQuoteToken.times(priceQuoteToken).toFormat(0)} </p>
                    </div>
